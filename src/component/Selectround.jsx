...
function Selectround({ notify }) {
  ...
  const handleSplit = () => {
    ...
    const unselectFilter = item?.form?.filter((data) => {
      const matchingItems = selectedItems?.filter((selectedItem) => selectedItem?._id === data?._id);
      return matchingItems?.length === 0; 
  });
  

   setUnselectedItems([...unselectFilter]);
        ...
        <th scope="col" class="px-6 py-3 sonm">
          CUTTING PRICE
            ...
            {unselectedItems.length > 0 && (
              <>
              <div style={{marginTop:"30px"}} ref={contonentPDF2} className='sara11 mt-2'>
                <div className="party_name">
                  <div className="party1">
                    <h2><span>PARTY NAME :  </span> {item?.client}</h2>
                  </div>
               ...
               <div className="select-row-btn">
               <div className="select-row-btn-flex">
                 <button onClick={generatePdf2} type="button" id="print">Print</button>
               </div>
             </div>
             </>
            )}
